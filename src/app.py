...
st.set_page_config(
    page_title="Fare-Sentinel | Sri Lanka",
    page_icon="🛺",
    layout="centered",
)
...
# ── Path helper — works regardless of where streamlit is run from ──────────────
...
def load_models():
    ...
    for v in ["bike", "tuk_tuk", "car"]:
        m = xgb.XGBRegressor()
        m.load_model(model_path(f"fare_model_{v}.json"))
        models[v] = m
    return models

@st.cache_data
def load_meta():
    with open(model_path("city_map.json"))     as f: city_map    = json.load(f)
    with open(model_path("fare_metrics.json")) as f: metrics     = json.load(f)
    with open(model_path("city_coords.json"))  as f: coords_raw  = json.load(f)
    with open(model_path("vehicle_features.json")) as f: vfeatures = json.load(f)
...
def build_features(dist, hour, dow, month, is_weekend, is_holiday,
                   precip, temp, humidity, origin, destination,
                   city_map, vehicle_features, vtype):
    """Build the exact feature vector each vehicle model expects."""
    ...
    # Return ONLY the features this vehicle's model was trained on — in exact order
    feats = vehicle_features[vtype]
    return pd.DataFrame([[all_feat[f] for f in feats]], columns=feats)

# ── Header ─────────────────────────────────────────────────────────────────────
st.markdown("## 🛺 Sri Lanka Ride Fare Predictor")
...
st.markdown(
    ...
    "Fare-Sentinel · XGBoost ML · Sri Lanka · Estimates only</div>",
    unsafe_allow_html=True,
)